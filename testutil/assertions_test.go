...
func TestAssertError(t *testing.T) {
	t.Run("success cases", func(t *testing.T) {
		...
		t.Run("expect error and get error", func(t *testing.T) {
			AssertError(t, errors.New("test error"), true, "test operation")
			// If we get here, the assertion passed
		})

		// Test expect no error and get no error
		t.Run("expect no error and get no error", func(t *testing.T) {
		...
		t.Run("complex operation name", func(t *testing.T) {
	...
	t.Run("edge cases", func(t *testing.T) {
		...
		t.Run("different error types", func(t *testing.T) {
			AssertError(t, ErrTestError, true, "using ErrTestError")
...
func TestAssertNoError(t *testing.T) {
	...
	t.Run("function availability", func(t *testing.T) {
...
func TestAssertExpectedError(t *testing.T) {
	t.Run("success cases", func(t *testing.T) {
		...
		t.Run("error present as expected", func(t *testing.T) {
			AssertExpectedError(t, errors.New("expected error"), "operation that should fail")
		})

		// Test with different error types
		t.Run("different error types", func(t *testing.T) {
			AssertExpectedError(t, ErrTestError, "test error operation")
	...
	t.Run("function availability", func(t *testing.T) {
		...
		AssertExpectedError(t, errors.New("test"), "test operation")
	})
}
...
func TestAssertErrorContains(t *testing.T) {
	...
	t.Run("different error types", func(t *testing.T) {
		t.Run("standard error", func(t *testing.T) {
			AssertErrorContains(t, ErrTestError, "test error", "test operation")
		})
...
func TestAssertionHelpers(t *testing.T) {
	t.Run("error types compatibility", func(t *testing.T) {
		// Test that all assertion functions work with ErrTestError
		AssertError(t, ErrTestError, true, "ErrTestError compatibility")
		AssertExpectedError(t, ErrTestError, "ErrTestError expected")
		AssertErrorContains(t, ErrTestError, "test", "ErrTestError contains")
	})

	t.Run("operation name handling", func(t *testing.T) {
		operations := []string{
			"",
			"simple operation",
		...
		for i, op := range operations {
			t.Run("operation_"+string(rune(i+'a')), func(t *testing.T) {
				...
				AssertErrorContains(t, errors.New("test error"), "test", op)
			})
		}
	})

	t.Run("error message variations", func(t *testing.T) {
		...
		for i, msg := range errorMessages {
			t.Run("error_message_"+string(rune(i+'a')), func(t *testing.T) {
				err := errors.New(msg)
				AssertError(t, err, true, "message test")
				AssertExpectedError(t, err, "message test")
				if msg != "" {
					// Only test contains if message is not empty
					AssertErrorContains(t, err, msg, "message test")
				}
			})
		}
	})
...
func BenchmarkStringOperations(b *testing.B) {
	...
	b.Run("contains_operation", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
	...
	b.Run("error_to_string", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_ = testErr.Error()
		}
...
func BenchmarkAssertionLogic(b *testing.B) {
	...
	b.Run("error_nil_check", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_ = testErr != nil
		}
	...
	b.Run("boolean_comparison", func(b *testing.B) {
		wantErr := true
		hasErr := testErr != nil
