...
func TestStructuredError_WithFilePath(t *testing.T) {
	err := &StructuredError{
		Type:    ErrorTypeFileSystem,
		Code:    "FILE_NOT_FOUND",
	...
	if err.FilePath != filePath {
		t.Errorf("Expected FilePath %q, got %q", filePath, err.FilePath)
	}
	...
	if err.FilePath != newPath {
		t.Errorf("Expected FilePath %q, got %q", newPath, err.FilePath)
	}
}

func TestStructuredError_WithLine(t *testing.T) {
	err := &StructuredError{
		Type:    ErrorTypeValidation,
		Code:    "SYNTAX_ERROR",
	...
	if err.Line != lineNum {
		t.Errorf("Expected Line %d, got %d", lineNum, err.Line)
	}
	...
	if err.Line != newLine {
		t.Errorf("Expected Line %d, got %d", newLine, err.Line)
	}
}
...
func validateStructuredErrorBasics(t *testing.T, err *StructuredError, errorType ErrorType, code, message, filePath string) {
	...
	if err.Type != errorType {
		t.Errorf("Expected Type %v, got %v", errorType, err.Type)
	}
	if err.Code != code {
...
func validateWrapErrorResult(t *testing.T, result *StructuredError, originalErr error, errorType ErrorType, code, message string) {
	...
	if result.Type != errorType {
		t.Errorf("Expected Type %v, got %v", errorType, result.Type)
	}
	if result.Code != code {
		t.Errorf("Expected Code %q, got %q", code, result.Code)
	}
...
func TestStructuredErrorIntegration(t *testing.T) {
	...
	// Test properties
	if err.FilePath != "/tmp/network.data" {
		t.Errorf("Expected FilePath %q, got %q", "/tmp/network.data", err.FilePath)
	}
	if err.Line != 42 {
		t.Errorf("Expected Line %d, got %d", 42, err.Line)
	}
	if len(err.Context) != 2 {
		t.Errorf("Expected context length 2, got %d", len(err.Context))
	}
