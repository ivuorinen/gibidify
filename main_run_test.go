	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(_ *testing.T) {
				...
				os.Args = []string{"gibidify", "-destination", "output.md", "-no-ui"}
			},
			expectError: true,
		...
		},
		{
			...
			setup: func(t *testing.T) {
				...
				flag.CommandLine = flag.NewFlagSet(os.Args[0], flag.ExitOnError)
				srcDir := t.TempDir()
				outFile, outPath := testutil.CreateTempOutputFile(t, "test_output.md")
				testutil.CloseFile(t, outFile)
				// Set args with invalid format
				os.Args = []string{
					"gibidify", "-source", srcDir, "-destination", outPath,
					"-format", "invalid", "-no-ui",
				}
			},
			expectError: true,
			errorSubstr: "parsing flags",
		},
		{
			...
			setup: func(t *testing.T) {
				...
				outFile, outPath := testutil.CreateTempOutputFile(t, "test_output.md")
				testutil.CloseFile(t, outFile)
				os.Args = []string{"gibidify", "-source", "/nonexistent/directory", "-destination", outPath, "-no-ui"}
			},
			expectError: true,
			errorSubstr: "parsing flags", // Flag validation catches this, not processing
		},
		{
			...
			setup: func(t *testing.T) {
				...
				os.Args = []string{"gibidify", "-source", srcDir, "-destination", outPath, "-no-ui"}
			},
			expectError: false,
		},
	}
